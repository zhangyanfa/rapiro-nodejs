...
({
	1 : [function (require, module) {
			...
			function Microphone(_options) {
				...
				this.recording = !1,
				this.requestedAccess = !1,
				this.sampleRate = 16e3,
				this.bufferUnusedSamples = new Float32Array(0),
				this.samplesAll = new Float32Array(2e7),
			...
			Microphone.prototype.onMediaStream = function (stream) {
				...
				this.mic || (this.mic = this.audioContext.createScriptProcessor(this.bufferSize, this.inputChannels, this.outputChannels)),
			...
			Microphone.prototype._exportDataBufferTo16Khz = function (bufferNewSamples) {
				...
				if (unusedSamples > 0) {
					...
					for (var i = 0; unusedSamples > i; ++i)
						buffer[i] = this.bufferUnusedSamples[i];
					for (i = 0; newSamples > i; ++i)
				...
				for (var filter = [ - .037935, -89024e-8, .040173, .019989, .0047792,  - .058675,  - .056487,  - .0040653, .14527, .26927, .33913, .26927, .14527,  - .0040653,  - .056487,  - .058675, .0047792, .019989, .040173, -89024e-8,  - .037935], samplingRateRatio = this.audioContext.sampleRate / 16e3, nOutputSamples = Math.floor((buffer.length - filter.length) / samplingRateRatio) + 1, pcmEncodedBuffer16k = new ArrayBuffer(2 * nOutputSamples), dataView16k = new DataView(pcmEncodedBuffer16k), index = 0, volume = 32767, nOut = 0, i = 0; i + filter.length - 1 < buffer.length; i = Math.round(samplingRateRatio * nOut)) {
					for (var sample = 0, j = 0; j < filter.length; ++j)
						sample += buffer[i + j] * filter[j];
					sample *= volume,
					dataView16k.setInt16(index, sample, !0),
					index += 2,
					nOut++
				}
				...
				if (remaining > 0)
					for (this.bufferUnusedSamples = new Float32Array(remaining), i = 0; remaining > i; ++i)
			...
			Microphone.prototype.encodeWav = function (samples, numChannels, sampleRate) {
				...
				return this.writeString(view, 0, "RIFF"),
	...
	5 : [function (require) {
			...
			$(document).ready(function () {
				...
				tokenGenerator.getToken(function (err, token) {
					...
					localStorage.setItem("playbackON", !1);
	...
	6 : [function (require, module, exports) {
			...
			exports.getModels = function (token) {
				...
				sttRequest.open("GET", modelUrl, !0),
				sttRequest.withCredentials = !0,
			...
			initSocket = exports.initSocket = function (options, onopen, onlistening, onmessage, onerror, onclose) {
				...
				socket.onclose = function (evt) {
					if (console.log("WS onclose: ", evt), 1006 === evt.code) {
						if (console.log("generator count", tokenGenerator.getCount()), tokenGenerator.getCount() > 1)
							throw $.publish("hardsocketstop"), new Error("No authorization token is currently available");
			...
			(function (global) {
				...
				exports.onFileProgress = function (options, ondata, running, onerror, onend, samplingRate) {
					...
					readChunk = function (evt) {
						if (offset >= fileSize)
							return console.log("Done reading file"), void onend();
						if (running()) {
							if (null != evt.target.error) {
								var errorMessage = evt.target.error;
								return console.log("Read error: " + errorMessage),
							...
							offset += len,
							ondata(buffer),
							samplingRate ? setTimeout(function () {
	...
	10 : [function (require, module, exports) {
			...
			showMetaData = function (alternative) {
				...
				if (confidenceNestedArray && confidenceNestedArray.length > 0)
					...
					timestampNestedArray && timestampNestedArray.length > 0 && timestampNestedArray.forEach(function (timestamp) {
			...
			exports.showJSON = function (msg, baseJSON) {
				...
				return baseJSON += json,
				baseJSON += "\n",
				"JSON" === $(".nav-tabs .active").text() && ($("#resultsJSON").append(baseJSON), baseJSON = "", console.log("updating json")),
				baseJSON
			};
			...
			exports.showResult = function (msg, baseString, model) {
				if (msg.results && msg.results.length > 0) {
					...
					if (text = text.replace(/%HESITATION\s/g, ""), text = text.replace(/(.)\1{2,}/g, ""), msg.results[0]["final"] && console.log("-> " + text), text = text.replace(/D_[^\s]+/g, ""), 0 === text.length || /^\s+$/.test(text))
						return baseString;
					var japanese = "ja-JP" === model.substring(0, 5) || "zh-CN" === model.substring(0, 5);
					msg.results && msg.results[0] && msg.results[0]["final"] ? (text = text.slice(0, -1), text = text.charAt(0).toUpperCase() + text.substring(1), japanese ? (text = text.trim() + "。", text = text.replace(/ /g, "")) : text = text.trim() + ". ", baseString += text, $("#resultsText").val(baseString), showMetaData(alternatives[0]), alternativePrototype.showAlternatives(alternatives)) : (text = japanese ? text.replace(/ /g, "") : text.charAt(0).toUpperCase() + text.substring(1), $("#resultsText").val(baseString + text))
				}
				return updateScroll(),
				updateTextScroll(),
				baseString
			...
			handleSelectedFile = exports.handleSelectedFile = function () {
				...
				function (token, file) {
					...
					r.onload = function () {
						var contentType;
						if ("fLaC" === r.result)
							contentType = "audio/flac", showNotice("Notice: browsers do not support playing FLAC audio, so no audio will accompany the transcription");
							...
							if ("OggS" !== r.result)
								return restoreUploadTab(), showError("Only WAV or FLAC or Opus files can be transcribed, please try another file format"), void localStorage.setItem("currentlyDisplaying", "false");
			...
			playSample = function () {
				...
				function (token, imageTag, sampleNumber, iconName, url) {
					...
					if (samplePlaying === sampleNumber)
						return console.log("HARD SOCKET STOP"), $.publish("socketstop"), localStorage.setItem("currentlyDisplaying", "false"), localStorage.setItem("samplePlaying", "false"), effects.stopToggleImage(timer, imageTag, iconName), effects.restoreImage(imageTag, iconName), void(running = !1);
					if ("record" === currentlyDisplaying)
						return void showError("Currently audio is being recorded, please stop recording before playing a sample");
					if ("fileupload" === currentlyDisplaying || "false" !== samplePlaying)
						return void showError("Currently another file is playing, please stop the file or wait until it finishes");
	...
	16 : [function (require, module, exports) {
			...
			exports.initRecordButton = function (ctx) {
				console.log("click initRecordButton");
					...
					return function (evt) {
						...
						return "sample" == currentlyDisplaying || "fileupload" == currentlyDisplaying ? void showError("Currently another file is playing, please stop the file or wait until it finishes") : (localStorage.setItem("currentlyDisplaying", "record"), void(running ? (console.log("Stopping microphone, sending stop action message"), recordButton.removeAttr("style"), recordButton.find("img").attr("src", "images/microphone.svg"), $.publish("hardsocketstop"), mic.stop(), running = !1, localStorage.setItem("currentlyDisplaying", "false")) : ($("#resultsText").val(""), console.log("Not running, handleMicrophone()"), handleMicrophone(token, currentModel, mic, function (err) {
										if (err) {
											...
											recordButton.css("background-color", "#d74108"), recordButton.find("img").attr("src", "images/stop.svg"), console.log("starting mic"), mic.record(), running = !0
									}))))
					}
				}
					())
